...
@click.command()
@click.option('--dcm_img', '-i', required=True, help="Directory containing the image series linked to the contour to convert")
@click.option('--dcm_rt_file', '-r', required=True, help="Dicom RTStruct file containing the contours to convert")
@click.option('--prefix', '-p', default='Struct_', help="Prefix for output files (e.g. Case_01_")
@click.option('--output_dir', '-od', default='.', help="Directory in which to place converted files")
@click.option('--output_img', '-oi', help="Output name of converted image series")
@click.option('--spacing', help="DICOM image spacing override with format x,y,z (0 indicates to leave as is, e.g. 0,0,3)")
