...
#                self.prob_unet.posterior.parameters(),
        ...
        for m in param_list:
            params += [
                {"params": m, "weight_decay": self.hparams.weight_decay, "lr": 1e-4}
            ]
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
           optimizer, lr_lambda=[lr_lambda_unet, lr_lambda_prob]
        )
    ...
    def training_step(self, batch, _):
        ...
        np.save("yyyy.npy", y.cpu().detach().numpy())
        ...
        self.kl_div = 1#loss["kl_div"].detach().cpu()

        for k in loss:
            if k == "loss":
                continue
            if k == "kl_div": continue
            self.log(
                k,
    ...
    def validation_step(self, batch, _):
        ...
        with torch.set_grad_enabled(False):
            ...
            np.save("img.npy", x.cpu().numpy())
            print(f"x: {x.shape}")
            print("y: " + str(y.shape))
            print(f"y1sum: {y.sum()}")
            np.save("seg.npy", y.cpu().numpy())
            ...
            print(f"seg1sum: {seg.sum()}")
            np.save("seg2.npy", seg.cpu().numpy())
