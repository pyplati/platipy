...
class ProbabilisticUnet(torch.nn.Module):
    ...
    def __init__(
        ...
        #)
        self.fcomb = Fcomb(filters_per_layer, 0, num_classes, no_convs_fcomb, ndims=ndims)
    ...
    def forward(self, img, seg=None, training=False):
        ...
        if self.use_structure_context:
            ...
            import numpy as np
            print(f"imgtype: {img.dtype}")
            print(f"imgshape: {img.shape}")
            print(f"segtype: {seg.dtype}")
            np.save("imgg.npy", img.cpu().numpy())
            img = torch.cat((img, seg), dim=1)
            np.save("imgg2.npy", img.cpu().numpy())
            np.save("segg.npy", seg.cpu().numpy())
    ...
    def reconstruct(self, use_posterior_mean=False, z_posterior=None, sample_x_stddev_from_mean=None):
        ...
        sample_x_stddev_from_mean = None
...
#                z_posterior = self.posterior_latent_space.rsample()
    ...
    def reconstruction_loss(
        ...
        y = segm
        pred = reconstruction
        np.save("predd.npy", pred.cpu().detach().numpy())
        ...
        np.save("yyy.npy", y.cpu().detach().numpy())
